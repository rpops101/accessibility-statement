...
accessibility-statement reads local files and writes local files. It has no network access, no daemon, no database and no credentials. That removes most of the usual surface. What remains:
...
| **Supply chain** | `@accessibility-statement/core` has **one** runtime dependency, justified in [DEPENDENCIES.md](packages/core/DEPENDENCIES.md). Small utilities are vendored rather than depended on. Dependency additions need explicit justification in review. |
...
- **accessibility-statement reporting a criterion as unevaluated.** That is the tool being honest about what the evidence supports.

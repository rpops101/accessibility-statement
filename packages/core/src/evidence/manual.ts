...
export function parseManualChecklist(content: string, path: string): ManualEntry[] {
  ...
  try {
    ...
    throw new EaaKitError({
      ...
      fix: 'Fix the YAML syntax, or regenerate the template with: eaa-kit init',
  ...
  if (!file || !Array.isArray(file.checklist)) {
    throw new EaaKitError({
      ...
      fix: 'Start from the template: eaa-kit init writes a commented manual.yaml.',
      docs: `${DOCS_BASE}/manual-checklist.md`,
    });
  }
...
export function manualChecklistTemplate(wcag: WcagStandard): string {
  const lines: string[] = [
    '# Manual accessibility checklist for eaa-kit.',

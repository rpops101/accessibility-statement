...
import { EaaKitError, DOCS_BASE } from '@eaa-kit/core';
import { flagBool, flagString, type ParsedArgs } from '../args.js';
import { resolvePacksDir } from '../resolve.js';

/**
 * `eaa-kit contrib scaffold-pack` / `scaffold-reader` (FR-CLI-4, REQ-PACK-3).
 *
  ...
  switch (sub) {
    ...
    default:
      throw new EaaKitError({
        ...
        fix: 'Use: eaa-kit contrib scaffold-pack --country xx | eaa-kit contrib scaffold-reader --name toolname',
...
function scaffoldPack(args: ParsedArgs): number {
  ...
  if (!/^[a-z]{2}$/.test(country)) {
    throw new EaaKitError({
      ...
      fix: 'Example: eaa-kit contrib scaffold-pack --country pt',
  ...
  process.stdout.write(
    `Scaffolded a jurisdiction pack for "${country}" in ${target}\n\n` +
      ...
      `       eaa-kit validate-pack ${target}\n` +
      `       npm run update-snapshots -w @eaa-kit/packs -- ${country}\n\n` +
...
# Validate as you go with:  eaa-kit validate-pack packages/packs/packs/${country}
schemaVersion: 1
country: ${country}
...
common:
  draftWatermark: "TODO: DRAFT — for human review. Generated by eaa-kit; this document does not constitute legal advice and must be reviewed by a responsible person before publication."
  reviewedBy: "TODO: Reviewed by"
  reviewedOn: "TODO: Reviewed on"
  generatedBy: "TODO: Generated with eaa-kit from recorded test evidence."
  status:
    pass: "TODO: Conforms"
...
A jurisdiction pack is **data**: it tells eaa-kit how an accessibility
...
- [ ] Validate: \`eaa-kit validate-pack ${target}\`
- [ ] Render and read it end to end — you are the first reader who can
      ...
      \`eaa-kit render statement --config ${target}/fixture/config.yaml \\
          --jurisdiction ${country} --lang ${lang}\`
- [ ] Commit snapshots: \`npm run update-snapshots -w @eaa-kit/packs -- ${country}\`
...
function scaffoldReader(args: ParsedArgs): number {
  ...
  if (!/^[a-z][a-z0-9-]*$/.test(name)) {
    throw new EaaKitError({
      ...
      fix: 'Example: eaa-kit contrib scaffold-reader --name wave',
  ...
  process.stdout.write(
    `Scaffolded an evidence reader for "${name}"\n\n  ${srcPath}\n  ${testPath}\n\n` +
      ...
      `     another tool's format; eaa-kit refuses to guess (FR-ING-5).\n` +
...
export const ${symbol}Reader: EvidenceReader = {
  ...
  // when eaa-kit cannot recognise a file.

> **BOZZA — da sottoporre a revisione umana. Generato da accessibility-statement; questo documento non costituisce consulenza legale e deve essere esaminato da una persona responsabile prima della pubblicazione.**

# Dichiarazione di accessibilità — Negozio online Esempio

Esempio S.r.l.: La presente dichiarazione di accessibilità si applica a Negozio online Esempio. La dichiarazione copre il negozio online pubblico su example.it.

- https://example.it/

## Stato di conformità

Negozio online Esempio è parzialmente conforme ai requisiti della norma EN 301 549 (WCAG, livello AA), a causa delle non conformità e delle esenzioni elencate di seguito.

## Contenuti non accessibili

I contenuti elencati di seguito non sono accessibili per i seguenti motivi:

- **Criterio WCAG 1.1.1 — Non-text Content** (Non conforme; clausola EN 301 549 9.1.1.1)
  - Images must have alternative text
- **Criterio WCAG 1.4.3 — Contrast (Minimum)** (Non conforme; clausola EN 301 549 9.1.4.3)
  - Elements must meet minimum color contrast ratio thresholds

## Contenuti non ancora valutati

I seguenti requisiti non sono ancora stati valutati:

- 1.2.1 — Audio-only and Video-only (Prerecorded)
- 1.2.2 — Captions (Prerecorded)
- 1.2.3 — Audio Description or Media Alternative (Prerecorded)
- 1.2.4 — Captions (Live)
- 1.2.5 — Audio Description (Prerecorded)
- 1.3.1 — Info and Relationships
- 1.3.2 — Meaningful Sequence
- 1.3.3 — Sensory Characteristics
- 1.3.4 — Orientation
- 1.3.5 — Identify Input Purpose
- 1.4.1 — Use of Color
- 1.4.2 — Audio Control
- 1.4.4 — Resize Text
- 1.4.5 — Images of Text
- 1.4.10 — Reflow
- 1.4.11 — Non-text Contrast
- 1.4.12 — Text Spacing
- 1.4.13 — Content on Hover or Focus
- 2.1.1 — Keyboard
- 2.1.2 — No Keyboard Trap
- 2.1.4 — Character Key Shortcuts
- 2.2.1 — Timing Adjustable
- 2.2.2 — Pause, Stop, Hide
- 2.3.1 — Three Flashes or Below Threshold
- 2.4.1 — Bypass Blocks
- 2.4.3 — Focus Order
- 2.4.4 — Link Purpose (In Context)
- 2.4.5 — Multiple Ways
- 2.4.6 — Headings and Labels
- 2.4.7 — Focus Visible
- 2.5.1 — Pointer Gestures
- 2.5.2 — Pointer Cancellation
- 2.5.3 — Label in Name
- 2.5.4 — Motion Actuation
- 3.1.2 — Language of Parts
- 3.2.1 — On Focus
- 3.2.2 — On Input
- 3.2.3 — Consistent Navigation
- 3.2.4 — Consistent Identification
- 3.3.1 — Error Identification
- 3.3.2 — Labels or Instructions
- 3.3.3 — Error Suggestion
- 3.3.4 — Error Prevention (Legal, Financial, Data)
- 4.1.1 — Parsing
- 4.1.3 — Status Messages

## Redazione della presente dichiarazione di accessibilità

La presente dichiarazione è stata redatta il 2026-07-01.

Metodo di valutazione: Autovalutazione con test automatizzati (axe-core) e checklist manuale.

## Feedback e recapiti

Se riscontra carenze di accessibilità o necessita di informazioni in un formato accessibile, la preghiamo di contattarci:

- <accessibilita@example.it>

## Procedura di attuazione

Se non è soddisfatto delle risposte ricevute, può rivolgersi all&#39;autorità competente:

- Agenzia per l&#39;Italia Digitale (AgID) — Autorità di vigilanza sui servizi ai sensi dell&#39;articolo 21, comma 1, del decreto legislativo 27 maggio 2022, n. 82
- Via Liszt 21, 00144 Roma, Italia
- +39 06 852641
- https://www.agid.gov.it/it/ambiti-intervento/accessibilita-usabilita

Può inoltre presentare un reclamo attraverso il canale ufficiale dedicato:

- Portale segnalazioni AgID — reclamo per problemi di accessibilità di un servizio digitale (articolo 21, comma 3, lettera a)
- https://segnalazioni.agid.gov.it/

---

Decreto legislativo 27 maggio 2022, n. 82 (attuazione della direttiva (UE) 2019/882) · Linee guida sull&#39;accessibilità dei servizi (EAA), AgID, versione 1.0 del 4 marzo 2026 · Legge 9 gennaio 2004, n. 4 (Legge Stanca) · Direttiva (UE) 2019/882 (European Accessibility Act) · EN 301 549

EN 301 549 v3.2.1 · WCAG 2.1 · Generato con accessibility-statement dalle evidenze di test registrate.

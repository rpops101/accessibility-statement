> **BORRADOR — pendiente de revisión humana. Generado por accessibility-statement; este documento no constituye asesoramiento jurídico y debe ser revisado por una persona responsable antes de su publicación.**

# Declaración de accesibilidad — Tienda en línea Ejemplo

Ejemplo S.L.: La presente declaración de accesibilidad se aplica a Tienda en línea Ejemplo. La declaración cubre la tienda en línea pública en example.es.

- https://example.es/

## Situación de cumplimiento

Tienda en línea Ejemplo es parcialmente conforme con los requisitos de la norma EN 301 549 (WCAG, nivel AA), debido a las no conformidades y exenciones que se enumeran a continuación.

## Contenido no accesible

El contenido que figura a continuación no es accesible por las siguientes razones:

- **Criterio WCAG 1.1.1 — Non-text Content** (No conforme; cláusula EN 301 549 9.1.1.1)
  - Images must have alternative text
- **Criterio WCAG 1.4.3 — Contrast (Minimum)** (No conforme; cláusula EN 301 549 9.1.4.3)
  - Elements must meet minimum color contrast ratio thresholds

## Contenido aún no evaluado

Los siguientes requisitos aún no se han evaluado:

- 1.2.1 — Audio-only and Video-only (Prerecorded)
- 1.2.2 — Captions (Prerecorded)
- 1.2.3 — Audio Description or Media Alternative (Prerecorded)
- 1.2.4 — Captions (Live)
- 1.2.5 — Audio Description (Prerecorded)
- 1.3.1 — Info and Relationships
- 1.3.2 — Meaningful Sequence
- 1.3.3 — Sensory Characteristics
- 1.3.4 — Orientation
- 1.3.5 — Identify Input Purpose
- 1.4.1 — Use of Color
- 1.4.2 — Audio Control
- 1.4.4 — Resize Text
- 1.4.5 — Images of Text
- 1.4.10 — Reflow
- 1.4.11 — Non-text Contrast
- 1.4.12 — Text Spacing
- 1.4.13 — Content on Hover or Focus
- 2.1.1 — Keyboard
- 2.1.2 — No Keyboard Trap
- 2.1.4 — Character Key Shortcuts
- 2.2.1 — Timing Adjustable
- 2.2.2 — Pause, Stop, Hide
- 2.3.1 — Three Flashes or Below Threshold
- 2.4.1 — Bypass Blocks
- 2.4.3 — Focus Order
- 2.4.4 — Link Purpose (In Context)
- 2.4.5 — Multiple Ways
- 2.4.6 — Headings and Labels
- 2.4.7 — Focus Visible
- 2.5.1 — Pointer Gestures
- 2.5.2 — Pointer Cancellation
- 2.5.3 — Label in Name
- 2.5.4 — Motion Actuation
- 3.1.2 — Language of Parts
- 3.2.1 — On Focus
- 3.2.2 — On Input
- 3.2.3 — Consistent Navigation
- 3.2.4 — Consistent Identification
- 3.3.1 — Error Identification
- 3.3.2 — Labels or Instructions
- 3.3.3 — Error Suggestion
- 3.3.4 — Error Prevention (Legal, Financial, Data)
- 4.1.1 — Parsing
- 4.1.3 — Status Messages

## Preparación de la presente declaración de accesibilidad

La presente declaración fue preparada el 2026-07-01.

Método de evaluación: Autoevaluación con pruebas automatizadas (axe-core) y lista de comprobación manual.

## Observaciones y datos de contacto

Si observa deficiencias de accesibilidad o necesita información en un formato accesible, póngase en contacto con nosotros:

- <accesibilidad@example.es>

## Procedimiento de aplicación

Si no está satisfecho con la respuesta a su solicitud, puede dirigirse a la autoridad competente:

- Unidad Técnica de Apoyo y Coordinación (UTAC) — Dirección General de Derechos de las Personas con Discapacidad, Ministerio de Derechos Sociales, Consumo y Agenda 2030
- Paseo del Prado 18-20, 28014 Madrid, España
- <utac@dsca.gob.es>
- https://www.dsca.gob.es/es/derechos-sociales/discapacidad/unidad-tecnica-apoyo-coordinacion-autoridades-vigilancia-materia

También puede dirigirse al órgano competente para tramitar y sancionar la reclamación:

- Autoridad de vigilancia designada por la comunidad autónoma correspondiente (artículo 27.3 de la Ley 11/2023), que es el órgano competente para investigar y sancionar

---

Ley 11/2023, de 8 de mayo, Título I (accesibilidad de determinados productos y servicios) · Real Decreto 143/2026, de 25 de febrero (Unidad Técnica de Apoyo y Coordinación) · Real Decreto Legislativo 1/2013, de 29 de noviembre (régimen sancionador supletorio, arts. 83-84) · Real Decreto 1112/2018 (accesibilidad de sitios web y aplicaciones móviles del sector público) · Directiva (UE) 2019/882 (European Accessibility Act) · EN 301 549

EN 301 549 v3.2.1 · WCAG 2.1 · Generado con accessibility-statement a partir de las evidencias de pruebas registradas.

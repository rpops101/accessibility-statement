> **ENTWURF — zur menschlichen Prüfung. Erstellt mit eaa-kit; dieses Dokument stellt keine Rechtsberatung dar und muss vor der Veröffentlichung von einer verantwortlichen Person geprüft werden.**

# Erklärung zur Barrierefreiheit — Beispiel Online-Shop

Beispiel GmbH: Diese Erklärung zur Barrierefreiheit gilt für Beispiel Online-Shop. Die Erklärung umfasst den öffentlichen Online-Shop unter example.de.

- https://example.de/

## Stand der Vereinbarkeit mit den Anforderungen

Beispiel Online-Shop ist wegen der nachstehend aufgeführten Unvereinbarkeiten und Ausnahmen teilweise mit den Anforderungen der EN 301 549 (WCAG, Stufe AA) vereinbar.

## Nicht barrierefreie Inhalte

Die nachstehend aufgeführten Inhalte sind aus folgenden Gründen nicht barrierefrei:

- **WCAG-Erfolgskriterium 1.1.1 — Nicht-Text-Inhalt** (Nicht konform; EN 301 549 Abschnitt 9.1.1.1)
  - Images must have alternative text
- **WCAG-Erfolgskriterium 1.4.3 — Kontrast (Minimum)** (Nicht konform; EN 301 549 Abschnitt 9.1.4.3)
  - Elements must meet minimum color contrast ratio thresholds

## Noch nicht bewertete Inhalte

Die folgenden Anforderungen wurden noch nicht bewertet:

- 1.2.1 — Reine Audio- und reine Videoinhalte (aufgezeichnet)
- 1.2.2 — Untertitel (aufgezeichnet)
- 1.2.3 — Audiodeskription oder Volltext-Alternative (aufgezeichnet)
- 1.2.4 — Untertitel (live)
- 1.2.5 — Audiodeskription (aufgezeichnet)
- 1.3.1 — Info und Beziehungen
- 1.3.2 — Bedeutungstragende Reihenfolge
- 1.3.3 — Sensorische Eigenschaften
- 1.3.4 — Ausrichtung
- 1.3.5 — Eingabezweck bestimmen
- 1.4.1 — Benutzung von Farbe
- 1.4.2 — Audio-Steuerelement
- 1.4.4 — Textgröße ändern
- 1.4.5 — Bilder eines Textes
- 1.4.10 — Umbruch (Reflow)
- 1.4.11 — Nicht-Text-Kontrast
- 1.4.12 — Textabstand
- 1.4.13 — Inhalt bei Hover oder Fokus
- 2.1.1 — Tastatur
- 2.1.2 — Keine Tastaturfalle
- 2.1.4 — Tastenkürzel mit einem Zeichen
- 2.2.1 — Zeiteinteilung anpassbar
- 2.2.2 — Pausieren, stoppen, ausblenden
- 2.3.1 — Dreimaliges Blitzen oder unterhalb des Schwellenwertes
- 2.4.1 — Blöcke umgehen
- 2.4.3 — Fokus-Reihenfolge
- 2.4.4 — Linkzweck (im Kontext)
- 2.4.5 — Verschiedene Methoden
- 2.4.6 — Überschriften und Beschriftungen
- 2.4.7 — Fokus sichtbar
- 2.5.1 — Zeigergesten
- 2.5.2 — Zeigerabbruch
- 2.5.3 — Beschriftung im Namen
- 2.5.4 — Bewegungsaktivierung
- 3.1.2 — Sprache von Teilen
- 3.2.1 — Bei Fokus
- 3.2.2 — Bei Eingabe
- 3.2.3 — Konsistente Navigation
- 3.2.4 — Konsistente Erkennung
- 3.3.1 — Fehlererkennung
- 3.3.2 — Beschriftungen oder Anweisungen
- 3.3.3 — Fehlerempfehlung
- 3.3.4 — Fehlervermeidung (rechtlich, finanziell, Daten)
- 4.1.1 — Parsen
- 4.1.3 — Statusmeldungen

## Erstellung dieser Erklärung zur Barrierefreiheit

Diese Erklärung wurde erstellt am 2026-07-01.

Bewertungsmethode: Selbstbewertung mit automatisierten Tests (axe-core) und manueller Checkliste.

## Feedback und Kontaktangaben

Sollten Ihnen Mängel in Bezug auf die barrierefreie Gestaltung auffallen oder benötigen Sie Informationen in einem barrierefreien Format, wenden Sie sich bitte an uns:

- <barrierefreiheit@example.de>

## Durchsetzungsverfahren

Wenn Sie mit den Antworten auf Ihre Anfrage nicht zufrieden sind, können Sie sich an die zuständige Marktüberwachungsstelle wenden:

- Marktüberwachungsstelle der Länder für die Barrierefreiheit von Produkten und Dienstleistungen (MLBF)
- Landesverwaltungsamt Sachsen-Anhalt, Ernst-Kamieth-Straße 2, 06112 Halle (Saale), Deutschland
- https://www.mlbf.de/

---

Barrierefreiheitsstärkungsgesetz (BFSG) · Verordnung zum Barrierefreiheitsstärkungsgesetz (BFSGV) · Richtlinie (EU) 2019/882 (European Accessibility Act) · EN 301 549

EN 301 549 v3.2.1 · WCAG 2.1 · Erstellt mit eaa-kit aus aufgezeichneten Testnachweisen.
